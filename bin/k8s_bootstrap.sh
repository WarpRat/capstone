...
helm upgrade google-application-credentials ./capstone/helm/charts/secrets -f ./capstone/helm/charts/secrets/values/storage-creds.yaml --install --set fileLit.gcs-application-credentials-file=gcs-key.json
helm upgrade gitlab-rails-storage ./capstone/helm/charts/secrets -f ./capstone/helm/charts/secrets/values/rails.yaml --install \
                                                    --set seclit.connection.google_project=${GOOGLE_CLOUD_PROJECT} \
                                                    --set seclit.connection.google_client_email=gitlab-storage-sa@${GOOGLE_CLOUD_PROJECT}.iam.gserviceaccount.com \
                                                    --set fileLit.connection.google_json_key_string=gcs-key.json
